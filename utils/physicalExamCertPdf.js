...
function drawDigitalAuthBlock(doc, { left, pageW, y, clinicianName, specialty, examDate, clinicianAddress, mroName, mrocc, cliaNumber, sigPath }) {
    ...
    if (sigPath && !sigPath.toLowerCase().endsWith('.webp')) {
        try {
            ...
            console.warn('Could not embed signature in physical exam cert:', err.message);
        }
    }
    currentY += rowH;
...
async function buildPhysicalExamCertPdf(id, options = {}) {
    const cert = await queryOne(
        `SELECT
            ...
            p.street1, p.street2, p.city, p.state, p.zipcode, p.email as patient_email,
         ...
         LEFT JOIN waiting_list wl ON pec.waiting_list_id = wl.id
